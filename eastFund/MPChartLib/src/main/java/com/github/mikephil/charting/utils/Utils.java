...
public abstract class Utils {
    ...
    public static Bitmap getRotatedBitmap(Bitmap sourceBitmap) {
        int width = sourceBitmap.getWidth();
        int height = sourceBitmap.getHeight();
        Matrix matrix = new Matrix();
//        matrix.postScale(1, -1);    //竖直旋转
        matrix.postScale(-1, 1);    //水平旋转
        // 围绕原地进行旋转
        Bitmap newBM = Bitmap.createBitmap(sourceBitmap, 0, 0, width, height, matrix, false);
        return newBM;
    }
    ...
    public static float getTextHeight(Paint paint, String text) {
        ...
        Log.e("Utils", "getTextHeight: " + height);
        return height;
    }
}
