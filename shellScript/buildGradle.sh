...
time gradle $buildMode $assembleMode $installMode -b "$androidStudioProjectDir/build.gradle" -c "$androidStudioProjectDir/settings.gradle" --daemon --parallel $syncMode -Dorg.gradle.jvmargs="-Xmx2048m -Xms512m -XX:MaxPermSize=1024m -XX:+HeapDumpOnOutOfMemoryError -Dfile.encoding=UTF-8" --continue --info --stacktrace 
