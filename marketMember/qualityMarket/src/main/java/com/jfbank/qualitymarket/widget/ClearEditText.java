...
public class ClearEditText extends EditText implements OnFocusChangeListener,
	...
	private void init() {
		setPadding(0, 0 , DensityUtil.dip2px(getContext(), 7.5f), 0);

		// 获取EditText的DrawableRight,假如没有设置我们就使用默认的图片
		mClearDrawable = getCompoundDrawables()[2];
		if (mClearDrawable == null) {
			mClearDrawable = getResources().getDrawable(R.drawable.ic_search_goods_activity_clear_text);
		}
	...
	public void onFocusChange(View v, boolean hasFocus) {
		if (hasFocus) {
			setClearIconVisible(getText().length() > 0);		//getText().length() > 0
		} else {
			setClearIconVisible(false);
		}
	}
	...
	public void onTextChanged(CharSequence s, int start, int count, int after) {
		setClearIconVisible(s.length() > 0);		//s.length() > 0
	}
