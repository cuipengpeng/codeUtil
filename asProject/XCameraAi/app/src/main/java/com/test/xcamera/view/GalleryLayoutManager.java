...
public class GalleryLayoutManager extends RecyclerView.LayoutManager implements RecyclerView.SmoothScroller.ScrollVectorProvider {
    ...
    private boolean mCallbackOnce = true;

    public GalleryLayoutManager(int orientation, boolean needScale) {
        mOrientation = orientation;
        mNeedScale = needScale;
    }


    public void setCallBackOnce(boolean b) {
        mCallbackOnce = b;
    }
            ...
            if (snap != null) {
                ...
                if (selectedPosition != mCurSelectedPosition) {
                    if (mCurSelectedView != null) {
                        mCurSelectedView.setSelected(false);
//                        if (mNeedScale) {
//                            mCurSelectedView.setScaleX(1.0f);
//                            mCurSelectedView.setScaleY(1.0f);
//                        }
                    }
                    mCurSelectedView = snap;
                    mCurSelectedView.setSelected(true);
//                    if (mNeedScale) {
//                        mCurSelectedView.setScaleX(1.3f);
//                        mCurSelectedView.setScaleY(1.3f);
//                    }
            ...
            if (mState == SCROLL_STATE_IDLE) {
                ...
                if (snap != null) {
                    ...
                    if (selectedPosition != mCurSelectedPosition) {
                        ...
                        if (mOnItemSelectedListener != null) {
                            mOnItemSelectedListener.onItemSelected(recyclerView, snap, mCurSelectedPosition);
                            Log.d(TAG, "onScrollStateChanged: onItemSelected 11111 " + mCurSelectedPosition);
                        }
                    } else if (mOnItemSelectedListener != null && mCurSelectedPosition != mItemFlag) {
                        mItemFlag = mCurSelectedPosition;
                        mCallbackOnIdle = false;
                        mOnItemSelectedListener.onItemSelected(recyclerView, snap, mCurSelectedPosition);
                        Log.d(TAG, "onScrollStateChanged: onItemSelected 22222 " + mCurSelectedPosition);
                    }
                } else {
...
//        ScaleAnimation scaleAnimation = (ScaleAnimation) AnimationUtils.loadAnimation(AppContext.getInstance(),
//                R.anim.fpv_setting_value_scale_anim);
