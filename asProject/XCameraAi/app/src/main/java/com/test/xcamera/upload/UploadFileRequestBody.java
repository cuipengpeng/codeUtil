    ...
    public void writeTo(BufferedSink sink) throws IOException {
        ...
        while ((len = fileInputStream.read(data)) != -1) {
            ...
            if (mProgressListener != null) {
                ...
                LoggerUtils.printLog("totalProgress="+mProgress+"--mFileCount="+mFileCount+"-----mFilePosition="+mFilePosition+"----the current " + currentLength + "----" + totalLength + "-----" + (currentLength * 1.0f/totalLength));
                mHandler.post(new Runnable() {
                    @Override
                    public void run() {
