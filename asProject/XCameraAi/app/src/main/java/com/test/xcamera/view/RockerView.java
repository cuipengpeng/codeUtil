...
public class RockerView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (ROCKER_BACKGROUND_MODE_PIC == mRockerBackgroundMode || ROCKER_BACKGROUND_MODE_XML == mRockerBackgroundMode) {
            ...
            canvas.drawBitmap(mRockerBitmap, null, dst, mRockerPaint);

//            RectF oval=new RectF(mCenterPoint.x-mAreaRadius , mCenterPoint.y-mAreaRadius , mCenterPoint.x + mAreaRadius, mCenterPoint.y + mAreaRadius);
            RectF oval1 = new RectF(0, 0, measuredWidth, measuredHeight);
            canvas.drawArc(oval1, startAngle, sweepAngle, false, paint1);

//            Rect src1 = new Rect(0, 0, measuredWidth, measuredHeight);
//            canvas.drawRect(src1,paint1);
...
//        tt.myy();
    ...
    private Point getRockerPositionPoint(Point centerPoint, Point touchPoint, float regionRadius, float rockerRadius) {
        ...
        // 回调 返回参数
        callBack(angle, Math.abs(lenX), Math.abs(lenY));
        startAngle = (int) temp_value - 15;
...
//        }
    ...
    private double radian2Angle(double radian) {
        ...
        System.out.println("获取的实际角度是多少。。。。" + tmp);
     ...
     * @param angle 摇动角度
     */
    private void callBack(double angle, float x, float y) {//
        if (null != mOnAngleChangeListener) {
            mOnAngleChangeListener.angle(angle, x, y);
        }
        if (null != mOnShakeListener) {
    ...
    public interface OnAngleChangeListener {
         ...
         */
        void angle(double angle, float x, float y);
