...
public class RingProgressBar extends View {
    ...
    public RingProgressBar(Context context, AttributeSet attrs, int defStyle) {
        ...
        textColor = mTypedArray.getColor(R.styleable.RingProgressBar_rTextColor, getResources().getColor(R.color.little_green_bg));//字体颜色
        ...
        ringColor = mTypedArray.getColor(R.styleable.RingProgressBar_rRingColor,getResources().getColor(R.color.color_eeeeee) );//外层初始圆环的颜色，默认透明 Color.GRAY
    ...
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        int ringWidth = getWidth() / 15;
        //1.画最外层的圆环
        int centre = getWidth() / 2; //获取圆心的x坐标
        int radius = centre - ringWidth / 2; //圆环的半径
        ...
        switch (style) {
            case STROKE: {//空心进度环
                ...
                canvas.drawArc(oval, -90, (float) (360 * progress / max), false, paint);  //根据进度画圆弧
                break;
            }
            case FILL: {//实心进度环
                paint.setStyle(Paint.Style.FILL_AND_STROKE);
                if (progress != 0) {
                    canvas.drawArc(oval, -90, (float) (360 * progress / max), true, paint);  //根据进度画圆弧
                }
                break;
            }
        }
