...
/**
 * author zxc
 * createTime 2019/10/14  12:54
 * <p>
 * 当前的 viewpager 是没有预加载功能的,使用这个控件只是用来进行左右滑动
 */
...
//        }
        ...
        for (i = 0; i < size; ++i) {
            ...
            if (child.getVisibility() != GONE) {
                lp = (ViewPager.LayoutParams) child.getLayoutParams();
                if (lp == null || !lp.isDecor) {
                    widthSpec = MeasureSpec.makeMeasureSpec((int) ((float) childWidthSize * 1), MeasureSpec.EXACTLY);
