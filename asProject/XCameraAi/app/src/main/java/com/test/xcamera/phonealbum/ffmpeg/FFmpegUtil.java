package com.test.xcamera.phonealbum.ffmpeg;
...
//                "\"[0:v]pad=iw*2:ih*2[a];[a][1:v]overlay=w[b];[b][2:v]overlay=0:h[c];[c][3:v]overlay=w:h\" %s";
        ...
        if(videoLayout == LinearLayout.VERTICAL){//vstack:垂直拼接
...
//        String reverseVideo = "ffmpeg -i %s -filter_complex [0:v]reverse[v];[0:a]areverse[a] -map [v] -map [a] %s";
     ...
     * @return 视频抽帧的命令行
     */
    public static  String[] videoToImage(String inputFile, int startTime, int duration, int frameRate, String targetFile, String prefix){
        //-ss：开始时间，单位为秒
        //-t：持续时间，单位为秒
        //-r：帧率，每秒抽多少帧
    ...
    static{
        System.loadLibrary("mediaInterface");
    }
