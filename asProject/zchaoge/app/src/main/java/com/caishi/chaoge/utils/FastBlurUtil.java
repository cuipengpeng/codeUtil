...
public class FastBlurUtil {
    ...
    public void blurUrlImage(final String url, final ImageView img) {
        new Thread(new Runnable() {
            @Override
            public void run() {
                //                        下面的这个方法必须在子线程中执行
                final Bitmap blurBitmap2 = GetUrlBitmap(url, 3);
                //                        刷新ui必须在主线程中执行
                BaseApplication.runOnUIThread(new Runnable() {//这个是我自己封装的在主线程中刷新ui的方法。
                    @Override
                    public void run() {
                        img.setScaleType(ImageView.ScaleType.CENTER_CROP);
                        img.setImageBitmap(blurBitmap2);
    ...
    //drawable转化为Bitmap或 BitmapFactory.decodeResource(getResources(), R.mipmap.aa_dong)
