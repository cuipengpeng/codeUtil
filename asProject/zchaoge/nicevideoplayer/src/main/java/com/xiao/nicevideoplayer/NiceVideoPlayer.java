...
public class NiceVideoPlayer extends FrameLayout
    ...
    public NiceVideoPlayer(Context context, AttributeSet attrs) {
        super(context, attrs);
        mContext = context;
        SAVED_DIR = mContext.getExternalFilesDir(Environment.DIRECTORY_PODCASTS);
        init();
    }

    private void init() {
        mContainer = new FrameLayout(mContext);
        mContainer.setBackgroundResource(R.drawable.im_video_loading);
        LayoutParams params = new LayoutParams(
                ViewGroup.LayoutParams.MATCH_PARENT,
                ViewGroup.LayoutParams.MATCH_PARENT);
        this.addView(mContainer, 0, params);
    }

    public void setUp(String url, boolean cache, Map<String, String> headers) {
        mUrl = url;
        mCache = cache;
        mHeaders = headers;
    }
...
//        mContainer.addView(mController, params);
     ...
     * 是否记录播放位置
     *
     * @param recordPlayPosition
     */
    public void start(boolean recordPlayPosition) {
        continueFromLastPosition = recordPlayPosition;
        start();
    }
...
//                    ((IjkMediaPlayer)mMediaPlayer).setOption(4, "framedrop", 1L);
    ...
    private void openMediaPlayer() {
        ...
        // 设置dataSource
        try {
            proxy = ProxyCacheManager.instance(mContext).getProxy(null);
            if (mCache && proxy != null && mUrl.startsWith("http") && !mUrl.contains("127.0.0.1") && !mUrl.contains(".m3u8")) {
...
//            mController.onPlayStateChanged(mCurrentState);
    ...
    public interface OnPlayCompletionListener {
        void onPlayCompletion(int playCount);
    }
    ...
    private IMediaPlayer.OnCompletionListener mOnCompletionListener
        ...
        public void onCompletion(IMediaPlayer mp) {
            ...
            if (mOnPlayCompletionListener != null) {
                mMediaPlayer.start();
                mOnPlayCompletionListener.onPlayCompletion(mPlayCount);
            }
//            mController.onPlayStateChanged(mCurrentState);
    ...
    private IMediaPlayer.OnInfoListener mOnInfoListener
        ...
        public boolean onInfo(IMediaPlayer mp, int what, int extra) {
            if (what == IMediaPlayer.MEDIA_INFO_VIDEO_RENDERING_START) {
                if (onHintSurface!=null){
                    onHintSurface.onInfo();
                }
                // 播放器开始渲染
                mCurrentState = STATE_PLAYING;
//                mController.onPlayStateChanged(mCurrentState);
...
//        if (isPlaying() || isBufferingPlaying() || isBufferingPaused() || isPaused()) {
        if (isPaused() && continueFromLastPosition) {
...
//        }
    ...
    public void setOnHintSurface(OnHintSurface onHintSurface) {
        this.onHintSurface = onHintSurface;
    }
    public interface OnHintSurface {
        void onInfo();
    }

    public void setOnVideoPrepared(OnVideoPrepared onVideoPrepared) {
