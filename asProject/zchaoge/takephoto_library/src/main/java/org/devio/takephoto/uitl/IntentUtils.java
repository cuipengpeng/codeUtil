...
public class IntentUtils {
    ...
    public static Intent getCaptureIntent(Uri outPutUri) {
        ...
        intent.addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION);
//        intent.setAction(MediaStore.INTENT_ACTION_STILL_IMAGE_CAMERA_SECURE);//设置Action为拍照
//        intent.setAction(MediaStore.INTENT_ACTION_STILL_IMAGE_CAMERA);//设置Action为拍照
...
//        intent.setAction(MediaStore.INTENT_ACTION_VIDEO_CAMERA);
    ...
    public static Intent getPickIntentWithGallery() {
        ...
        intent.setType("video/*;image/*");//从所有图片中进行选择
//        intent.setType("*/*");
        return intent;
    }
     ...
     * @return
     */
    public static Intent getPickIntentWithPicture() {
        Intent intent = new Intent();
        intent.setAction(Intent.ACTION_PICK);//Pick an item from the data
