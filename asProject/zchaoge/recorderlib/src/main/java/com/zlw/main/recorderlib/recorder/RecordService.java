...
public class RecordService extends Service {
    ...
    private final static int ACTION_MAKE_TEMPORARY_PLAY_RECORD = 5;

    private final static int ACTION_CLEAR_FILE = 6;
        ...
        if (bundle != null && bundle.containsKey(ACTION_NAME)) {
            switch (bundle.getInt(ACTION_NAME, ACTION_INVALID)) {
                ...
                case ACTION_PAUSE_RECORD:
                    doPauseRecording();
                    break;
                case ACTION_MAKE_TEMPORARY_PLAY_RECORD:
                    doMakeTemporaryPlayRecording();
                    break;
                case ACTION_CLEAR_FILE:
                    clearFile();
                    break;
                default:
                    break;
    ...
    public static void makeTemporaryPlay(Context context) {
        Intent intent = new Intent(context, RecordService.class);
        intent.putExtra(ACTION_NAME, ACTION_MAKE_TEMPORARY_PLAY_RECORD);
        context.startService(intent);
    }

    public static void clearFile(Context context) {
        Intent intent = new Intent(context, RecordService.class);
        intent.putExtra(ACTION_NAME, ACTION_CLEAR_FILE);
        context.startService(intent);
    }
    ...
    public static void setRecordTemporaryPlayListener(RecordTemporaryPlayListener recordTemporaryPlayListener) {
        RecordHelper.getInstance().setRecordTemporaryPlayListener(recordTemporaryPlayListener);
    }
    ...
    private void doMakeTemporaryPlayRecording() {
        RecordHelper.getInstance().makeTemporaryPlayFile();
    }
    private void clearFile() {
        RecordHelper.getInstance().clearFile();
    }
