...
public class StickerFilter extends AFilter {
    ...
    protected void onDrawArraysAfter(Frame frame) {
        if (stickerTexture == -1 && bitmap != null) {
            stickerTexture = TextureUtil.loadTexture(bitmap, TextureUtil.NO_TEXTURE, false);
//            stickerTexture = OpenGlUtils.loadTexture(bitmap, frame.getTextureId(), false);
//            stickerTexture = frame.getTextureId();
        }
        ...
        if(stickerCube == null){
            ...
            int width = frame.getPreviewWidth();
            int height = frame.getPreviewHeight();
