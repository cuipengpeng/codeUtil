...
public class FingerprintUtils {
    ...
    public FingerprintUtils(Context context) {
        ...
        LogUtils.d("fingerprint isSupport: " + isSupport);
        initCryptoObject();
    }
    ...
    private void initCryptoObject() {
        try {
            ...
            LogUtils.d("create cryptoObject failed!");
        }
    }
    ...
    private void startAuthenticate(final FingerprintManager.CryptoObject cryptoObject) {
        prepareData();
        mState = AUTHENTICATING;
        try {
            if (ActivityManager.getInstance().currentActivity() instanceof CommonActivity) {
                ((CommonActivity) ActivityManager.getInstance().currentActivity()).checkPermissions(new CommonActivity.CheckPermListener() {
                    @Override
                    public void superPermission() {
                        mFingerprintManager.authenticate(cryptoObject, mCancellationSignal, 0, mAuthCallback, null);
                    }
                }, R.string.permission_tip, Manifest.permission.USE_FINGERPRINT);
            }
    ...
    private void notifyAuthenticationError(int errMsgId, CharSequence errString) {
        LogUtils.d("onAuthenticationError, errId:" + errMsgId + ", err:" + errString + ", retry after 30 seconds");
        if (null != mFpResultListener && null != mFpResultListener.get()) {
    ...
    private void notifyAuthenticationFailed(int msgId, String errString) {
        LogUtils.d("onAuthenticationFailed, msdId: " + msgId + " errString: " + errString);
        if (null != mFpResultListener && null != mFpResultListener.get()) {
    ...
    private void onFailedRetry(int msgId) {
        mFailedTimes++;
        LogUtils.d("on failed retry time " + mFailedTimes);
        cancelAuthenticate();
