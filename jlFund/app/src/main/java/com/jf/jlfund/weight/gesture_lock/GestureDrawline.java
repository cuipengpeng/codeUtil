package com.jf.jlfund.weight.gesture_lock;
...
import com.jf.jlfund.bean.GesturePoint;
...
public class GestureDrawline extends View {
	...
	public GestureDrawline(Context context, List<GesturePoint> list, boolean isVerify,
		...
		super(context);
		screenDispaly = DensityUtil.getWidthHeight();
		...
		switch (event.getAction()) {
			case MotionEvent.ACTION_DOWN:
				...
				if (currentPoint != null) {
					currentPoint.setPointState(ConstantsUtil.POINT_STATE_SELECTED);
			...
			case MotionEvent.ACTION_MOVE:
				...
				if (pointAt == null || currentPoint.equals(pointAt) || ConstantsUtil.POINT_STATE_SELECTED == pointAt.getPointState()) {
					...
					pointAt.setPointState(ConstantsUtil.POINT_STATE_SELECTED);

					// 判断是否中间点需要选中
					GesturePoint betweenPoint = getBetweenCheckPoint(currentPoint, pointAt);
					if (betweenPoint != null && ConstantsUtil.POINT_STATE_SELECTED != betweenPoint.getPointState()) {
						// 存在中间点并且没有被选中
						...
						// 设置中间点选中
						betweenPoint.setPointState(ConstantsUtil.POINT_STATE_SELECTED);
						// 赋值当前的point;
						currentPoint = pointAt;
					} else {
	...
	final class ClearDrawBitmapRunnable implements Runnable {
		public void run() {
			...
			for (GesturePoint p : list) {
				p.setPointState(ConstantsUtil.POINT_STATE_NORMAL);
			}
			// 清空保存点的集合
			lineList.clear();
			// 重新绘制界面
	...
	private void drawErrorPasswordPath(boolean correctPpassword) {
		...
		for (Pair<GesturePoint, GesturePoint> pair : lineList) {
			if(correctPpassword){
				...
				pair.first.setPointState(ConstantsUtil.POINT_STATE_CORRECT);
				pair.second.setPointState(ConstantsUtil.POINT_STATE_CORRECT);
			}else {
				paint.setColor(context.getResources().getColor(R.color.appRedColor));
				pair.first.setPointState(ConstantsUtil.POINT_STATE_WRONG);
				pair.second.setPointState(ConstantsUtil.POINT_STATE_WRONG);
			}
			mCanvas.drawLine(pair.first.getCenterX(), pair.first.getCenterY(),
					pair.second.getCenterX(), pair.second.getCenterY(), paint);// 画线
		}
