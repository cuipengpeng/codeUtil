...
public class JsResponseInterface {
    ...
    public void jsHandlerFunc(String path) {
        if (FaceDetectUtil.getImageListName(path) != null && FaceDetectUtil.getImageListName(path).size() == 4) {
            ...
            final Call<String> call = HttpRequest.uploadFileWithHeader(mActivity, HttpRequest.ONE_CARD_API_URL + HttpRequest.ONCARD_API_FACE_IDENTIFY, header, NetUtil.toParamsFiles(postFormBuilder, "multipart/form-data"), new AsyncResponseCallBack() {
                @Override
