...
import com.huoyanzhengxin.zhengxin.print.App;
...
//	    return compress(bitmap, maxSize); // 这里再进行质量压缩的意义不大，反而耗资源，删除
     ...
     * @throws IOException
     */
    public static String compressAndGenImage(Bitmap image, String outPath, int maxSize) throws IOException {
        ByteArrayOutputStream os = new ByteArrayOutputStream();
        // scale
        int options = 100;
    ...
    public static String compressAndGenImage(String imgPath,String outPath, int maxSize) throws IOException {
        return compressAndGenImage(getBitmap(imgPath), outPath, maxSize);
    }
    public static String compressAndGenImage(String imgPath, int maxSize) throws IOException {
        return compressAndGenImage(getBitmap(imgPath), PhotoUtils.getFile(AppContext.mContext).getPath(), maxSize);
    }
